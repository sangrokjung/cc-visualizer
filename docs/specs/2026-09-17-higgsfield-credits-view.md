...
- 순수 로직(`higgsfieldCredits.ts`) 단위 테스트: 주기 실측(2건·1건·0건), D-day 경계(당일·자정 넘김), 주기 사용량 합산, 소멸 예측, 모델별 집계.
- 뷰 렌더 테스트: 정상 데이터·CLI 실패 상태.
- 실제 QA: `npm run tauri dev`로 앱을 띄워 뷰를 눈으로 확인.
