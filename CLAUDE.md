...
- **스케줄링 클래스 (2026-09-23)**: LaunchAgent plist에 `ProcessType=Interactive`가 필수다. 이 호스트는 배치 프로세스로 load 40~200이 상시라 Standard 클래스면 메인 스레드가 굶어 갱신 한 번에 CPU 100ms/벽시계 2~6초가 걸렸다(`DASHBOARD-REFRESH … elapsed= cpu=` 로그로 판별). 템플릿(`menubar/com.qjc.cc-menubar.plist.template`)과 설치된 plist 둘 다 유지.
